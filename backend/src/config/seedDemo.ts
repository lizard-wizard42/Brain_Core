...
export async function seedDemoData(): Promise<void> {
  ...
  const welcome = await query<PageId>(
    `INSERT INTO pages (title, slug, type, icon, is_section, content, sort_order, tags)
     VALUES ($1, $2, 'note', $3, TRUE, $4, 0, $5)
     RETURNING id`,
    [
  ...
  await query(
    `INSERT INTO pages (parent_page_id, title, slug, type, icon, content, sort_order, status, due_date, tags)
     VALUES ($1, $2, $3, 'note', $4, $5, 0, 'em andamento', CURRENT_DATE + INTERVAL '7 days', $6)`,
    [
  ...
  await query(
    `INSERT INTO pages (title, slug, type, icon, content, sort_order, tags)
     VALUES ($1, $2, 'infinite', $3, $4, 1, $5)`,
    ['Mapa visual', 'mapa-visual', '🧠', JSON.stringify({ tldraw: true, version: 1, data: null }), ['demo', 'visual']],
  );
